/* **************************************************************
*****************************************************************
TELM606CCONSTACTLAYER.CPP - Runs TEM for a single grid cell.  The
...
#include "telm606cConstActLayer.h"
                                    ...
                                    const int& pdm )
{
  ...
  tem.soil.setCONSTACTLAYER( cohort[pichrt].constActLayer[pdm],
                             pdm );
...
void TEMelmnt60::initializeCohortTEMState( const int& pichrt )
{
  ...
  for( dm = 0; dm < CYCLE; ++dm )
  {
    cohort[pichrt].constActLayer[dm] = ZERO;
  }
...
//  float dumflt;
  ...
  for( dm = 0; dm < CYCLE; ++dm )
  {  
    ifstate >> cohort[pichrt].constActLayer[dm];
  }
...
void TEMelmnt60::saveTEMCohortState( const int& pichrt )
{
  ...
  for( dm = 0; dm < CYCLE; ++dm )
  {  
    cohort[pichrt].constActLayer[dm] = tem.soil.getCONSTACTLAYER( dm );
  }
                                    ...
                                    ElmntCohort60& targetchrt )
{
  ...
  for( dm = 0; dm < CYCLE; ++dm )
  {  
    targetchrt.constActLayer[dm] = firstchrt.constActLayer[dm];
  }
                                   ...
                                   const int& pichrt )
{
  ...
  for( dm = 0; dm < CYCLE; ++dm )
  {  
    ofstate << cohort[pichrt].constActLayer[dm];
  }
